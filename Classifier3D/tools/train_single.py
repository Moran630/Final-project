...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # 娣诲姞
print(BASE_DIR)
...
def train(rank, local_rank, device, args):
    ...
    elif args.single_loss == 'mutliloss':
        criterion = MultiTaskLoss_withoutany(label_names)
    else:
        criterion = SingleTaskLoss(label_names)
    ...
    for epoch in range(start_epoch, args.epochs):
        if args.data_resample:
            ...
            with torch_distributed_zero_first(rank):
                ...
                # train_dataset.img_files = train_dataset.resample_data_single()
        ...
        for step, (img, label_infos) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            with autocast(dtype=torch.bfloat16, enabled=args.amp,device_type='cuda',cache_enabled=False):
                output = model(img)
            ...
            train_total_loss.update(loss.detach().item(), img.size(0))
            for name in label_names:
                train_losses[name].update(losses_dict[name].detach().item(), img.size(0))
