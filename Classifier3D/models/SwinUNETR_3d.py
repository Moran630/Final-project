...
class SWINUNETR_CLS(nn.Module):
    ...
    def __init__(
        ...
        self.bowel_layer = nn.Sequential(
            nn.Linear(feature_size*31*1*1*1, 128),
            nn.Dropout(0.2),
            nn.Linear(128, 1)
        )
        ...
        # self.out_layer = nn.Linear(feature_size*31*1*1*1, self.num_classes)
    ...
    def forward(self, x, return_feat=False):
        ...
        x0 = F.adaptive_avg_pool3d(hidden_states_out[0], [1, 1, 1])
        x1 = F.adaptive_avg_pool3d(hidden_states_out[1], [1, 1, 1])
        x2 = F.adaptive_avg_pool3d(hidden_states_out[2], [1, 1, 1])
        x3 = F.adaptive_avg_pool3d(hidden_states_out[3], [1, 1, 1])
        x4 = F.adaptive_avg_pool3d(hidden_states_out[4], [1, 1, 1])
...
if __name__ == '__main__':
    ...
    print('inital sucessed')
    # tmp = torch.randn(1, 1, 196, 196, 196).cuda()
