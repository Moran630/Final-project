...
class DefaultLoader(data.Dataset):
    ...
    def db_loader(self):
        ...
        for key, value in txn.cursor():
            ...
            if self.data_region == 'all':
                img_file = os.path.join(self.root_dir, key + '_croped.nii.gz')
            else:
                img_file = os.path.join(self.root_dir, 'train_images_nii_croped_z_1.0_' + self.data_region, key + '_croped.nii.gz')
    ...
    def resample_data(self):
        ...
        injury_resample_ratio = {'bowel': 10, 'extravasation': 1, 'kidney': 1, 'liver': 1, 'spleen': 1}
        for name, img_files in binary_dict.items():
            if 'healthy' in name:
                continue
    ...
    def resample_data_single(self):
        ...
        for label, img_file_list in files_dict.items():
            if label == 0:
                neg_resample = random.sample(img_file_list, 3 * pos_num)
